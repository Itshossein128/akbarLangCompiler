...
class CodeGenerator {
  constructor() {
    this.code = [];
    this.indentLevel = 0;
    this.tempVars = new Map(); // Store temporary variables with their types
    this.userVars = new Set(); // Track user-defined variables
  }
  ...
  generate(instructions) {
    ...
    for (const instr of instructions) {
      if (instr.op === 'LOAD') {
        ...
        this.registerUserVar(instr.args[1]);
      }
    }
    ...
    for (const name of this.userVars) {
      this.addCode(`int ${name};`);
    }
    this.addCode('');
  ...
  generateMainBegin() {
    ...
    // Declare temporary variables with their proper types
